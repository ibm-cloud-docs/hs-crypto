...
# Why can't I rotate master keys using recovery crypto units?
{: #troubleshoot-master-key-rotation-recovery-crypto-units}
{: troubleshoot}
{: support}

When you run the `ibmcloud tke auto-mk-rotate` command to rotate master keys using recovery crypto units, you might be not able to complete the rotation.
{: shortdesc}
...
For detailed instructions on how to rotate the master key, see [Rotating master keys using recovery crypto units](/docs/hs-crypto?topic=hs-crypto-rotate-master-key-cli-recovery-crypto-unit).
