...
For the complete command reference, see [Trusted Key Entry CLI plug-in reference](/docs/hs-crypto?topic=hs-crypto-cli-plugin-tke_cli_plugin). For detailed instructions, see [Initializing service instances with {{site.data.keyword.cloud_notm}} Trusted Key Entry CLI plug-in](/docs/hs-crypto-cli-plugin/hs-crypto-cli-plugin-tke_cli_plugin).
