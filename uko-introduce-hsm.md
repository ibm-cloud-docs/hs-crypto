...
| Additional configuration file   | Required. You need to configure all of the parameters that include user roles in this file to ensure the successful deployment of the PKCS #11 library. For more information, see the [configuration file template](/docs/hs-crypto?topic=hs-crypto-uko-set-up-pkcs-api#uko-step3-setup-configuration-file).  |  Not required. The parameters that are needed to connect the {{site.data.keyword.hscrypto}} cloud HSM can be set up along with the application code. |
