...
## Step 1. Create an import token
{: #tutorial-import-create-token}

With your service credentials, you can start interacting with the key management APIs to create and bring your encryption keys to the service.
...
## Step 2. Retrieve the import token
...
* Encrypt the generated key with the key management API:
  ...
  * Encrypt the generated key with the CLI:
...
<!-- ## Step 1. Create a {{site.data.keyword.hscrypto}} service instance
{: #tutorial-import-provision-service}

After you set up an {{site.data.keyword.cloud_notm}} account, complete the following steps to provision a {{site.data.keyword.hscrypto}} service instance.

1. From the command line, run the following command to log in to {{site.data.keyword.cloud_notm}} with the [{{site.data.keyword.cloud_notm}} CLI](/docs/cli?topic=cli-getting-started){: external}.
...
2. Select the account and resource group where you would like to create a {{site.data.keyword.hscrypto}} service instance.

    In this tutorial, you'll interact with the Dallas region. If you're logged into a different region, be sure to set Dallas as your target region by running the following command.
...
3. Provision an instance of {{site.data.keyword.hscrypto}} within that account and resource group.
    ...
    ```sh
     ibmcloud resource service-instance-create "import-keys-demo" hs-crypto standard us-south
    ```
    {: pre}
...
2. Set the directory to be used to store the master key part files and signature part key files. The directory needs to be set in the variable CLOUDTKEFILES. For example:
...
3. Select the crypto units to be worked on:

  Your crypto unit is in the *imprint mode* state.
  {: note}

  1. Display the crypto units that are assigned to you:
    ...
    Note the `SELECTED` column in the output of that command:
  ...
  2. Select the crypto unit with the following command：
  ...
  3. At the prompt, enter `1` to select that crypto unit. This will confirm
...
4. Create a signature key for the admin user to sign the operation of creating the master key.

  1. Create a signature key:
    ...
    When prompted, enter a username and password to protect the signature key. For example, you can choose: `admin / passw0rd`.

  2. Select the signature key created as the signature key to perform future signing operations with:
  ...
  3. Add the admin user (signature key) to the crypto unit so it can perform operations:
    ...
    Enter `1` to choose the admin user created, and enter the signature key's password again.

  4. Exit imprint mode and set the signature thresholds:
    ...
    When prompted, enter `1` for the signature threshold and revocation signature threshold, and then enter the signature key's password again.


  5. Create master key part files:

    1. Create the first master key part:
      ...
      You can provide `key 1` as the key description, and `passw0rd` as the password.
    ...
    2. You need at least two master key parts, so repeat the previous step:
      ...
      The two master key parts and the signature key are in the `CLOUDTKEFILES` directory of your workstation.

    3. Load the master key parts and the signature key on the same workstation to add the master key parts to a crypto unit. To do this, run:
      ...
      Enter `1 2` at the prompt. Enter passwords for all three keys.

    4. Commit this master key register:
      ...
      Because this is an operation against the crypto unit, you need to enter the password for
      the signature key of the administrator with access to the crypto
      unit. Now you'll see a fully-committed key:
    ...
    5. To activate this master key, run the following command:
      ...
      Type `y` when prompted. Again, enter password of the signature key of the administrator with access to the crypto unit.
  ...
  Success! You're now set with the service credentials that you need to authenticate to the {{site.data.keyword.hscrypto}} key management API. Continue to the next step.

## Step 3. Set up the {{site.data.keyword.hscrypto}} key management API
{: #tutorial-import-configure-api}

Now that you've provisioned an instance of {{site.data.keyword.hscrypto}} and the master key is ready, you're ready to start using the API.

{{site.data.keyword.hscrypto}} provides a graphical user interface and a REST API to create, track, and manage encryption keys. The key management API requires a valid {{site.data.keyword.cloud_notm}} IAM token and an instance ID to authenticate with the service.

In this step, you'll use the {{site.data.keyword.cloud_notm}} CLI to gather the authentication credentials you need to start interacting with {{site.data.keyword.hscrypto}} key management APIs. To retrieve and prepare your credentials for later steps, you'll also set the credentials as environment variables from the command line.

1. From the command line, set the key management API endpoint as an environment variable.

    ```sh
    export HPCS_API_URL=https://api.us-south.hs-crypto.cloud.ibm.com:8069
    ```
    {: pre}
...
4. Optional: Verify that the environment variables are set correctly by printing them from the command line.
    ...
    Success! You're now set with the service credentials that you need to authenticate to the key management API. Continue to the next step. -->
