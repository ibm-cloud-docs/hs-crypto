...
您可以使用 {{site.data.keyword.cloud}} {{site.data.keyword.hscrypto}}，透過 {{site.data.keyword.hscrypto}} GUI 或利用 {{site.data.keyword.hscrypto}} API 以程式設計方式來保護現有根金鑰的安全。
{: shortdesc}
...
[在建立服務的實例之後](/docs/services/hs-crypto/provision.html)，請完成下列步驟以使用 {{site.data.keyword.hscrypto}} GUI 來新增現有的根金鑰。
...
1. 使用下列 cURL 指令，來呼叫 [{{site.data.keyword.hscrypto}} API ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}/apidocs/hs-crypto){: new_window}。
    ...
    <table>
      ...
      <tr>
        ...
        <caption style="caption-side:bottom;">表 1. 說明使用 {{site.data.keyword.hscrypto}} API 新增根金鑰所需的變數</caption>
    </table>
...
2. 選用項目：執行下列呼叫來瀏覽 {{site.data.keyword.hscrypto}} 服務實例中的金鑰，確認已新增金鑰。
...
**附註：**當您將現有根金鑰新增至服務時，金鑰會保留在 {{site.data.keyword.hscrypto}} 的範圍內，而且無法擷取其金鑰資料。
