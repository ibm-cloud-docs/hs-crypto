...
|---------------|--------------|-----------------|
| Interface implementation  | Stateful interface. The result of your API request may vary depending on the implicit state such as session state and user login state. In the stateful case, data is stored on the host. | Stateless interface. The result of your API request always stays the same. In the stateless case, no data is stored on the host. |
