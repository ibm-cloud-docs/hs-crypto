...
3. Create a Terraform configuration file `main.tf` in the same folder as `versions.tf`. In this file, you add the configurations to perform the corresponding actions.
    ...
    | `signature_server_url` | **Optional**. The URL and port number where the signing service is running. If you are using a third-party signing service to provide administrator signature keys, you need to specify this parameter. |Table 1. Supported parameters for provisioning a service instance with Terraform
