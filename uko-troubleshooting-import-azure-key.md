...
After you create a key in {{site.data.keyword.hscrypto}} with {{site.data.keyword.uko_full_notm}} and assign it to a keystore of Azure Key Vault, you cannot find the key that is listed in the connected Key Vault instance in the Azure cloud or use it for encryption.
{: tsSymptoms}
...
Create a key from the Azure Key Vault UI based on the following key settings. And then, reassign keystores for keys that you create from the {{site.data.keyword.hscrypto}} UI again. For detailed instructions, see [Editing key details with the {{site.data.keyword.cloud_notm}} console](/docs/hs-crypto?topic=hs-crypto-edit-kms-keys).
{: tsResolve}
