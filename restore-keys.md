...
Because purged keys are completely inaccessible and destroyed, there is technically no _Purged_ key state. However, it can be useful to think of _Purged_ as being a state because nonexistence is part of the lifecycle of a key.
{: note}
...
4. Check whether there is an icon next to the **Destroyed** state. The icon indicates that the key can be restored. If you click the icon, it shows the expiration date of the restoration action. After the expiration date, you will not be able to restore the key.
...
5. Select the key that you want to restore and click the overflow icon (...) to open a list of options for the key.
