...
Before you purge keys, make sure you understand [the concept of deleting and purging keys](/docs/hs-crypto?topic=hs-crypto-delete-purge-keys) and review the [considerations](/docs/hs-crypto?topic=hs-crypto-delete-purge-keys#delete-purge-keys-considerations).

## Purging keys in the console
{: #purge-keys-gui}

If you prefer to purge a key by using a graphical interface, you can use the {{site.data.keyword.cloud_notm}} console by completing the following steps:
...
5. Select the key that you want to purge and make sure its state is _Destroyed_. Click the overflow (⋯) icon to open a list of options for the key.
6. From the options menu, click **Purge key**.
7. Check the box to confirm the action and click **Purge key**.

You are not able to purge a key if you perform the action within four hours after its deletion.
{: note}
