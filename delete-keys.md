...
3. Run the following cURL command to permanently delete the key and its contents.
    ...
    {{site.data.keyword.hscrypto}} blocks the deletion of a key
    that's protecting a cloud resource, such as a {{site.data.keyword.cos_full_notm}} bucket. You
    ...
    [checking the registration details](/docs/hs-crypto?topic=hs-crypto-view-protected-resources)
    for the key. Then, you must contact an account owner to remove the retention
    policy on each resource that is associated with the key before you can delete
    the key.
    {: important}
    ...
    3. Run the following cURL command to force delete the key and its contents.
        ...
        [key management API reference doc](/apidocs/hs-crypto){: external}.
