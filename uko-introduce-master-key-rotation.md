...
- You can still create internal KMS keys. However, an **Out of sync** flag can be displayed after the key creation. To sync each of these keys, after the master key rotation is complete, select **Show details** on the Actions  ![Actions icon](../icons/action-menu-icon.svg "Actions")  menu and then click **Sync keys**.  
